...
def write_ply1(path, frame_num, dim, num, pos, rho_0):
    if dim == 3:
        list_pos = [(pos[i, 0], pos[i, 1], pos[i, 2], rho_0[i]) for i in range(num)]
    elif dim == 2:
        list_pos = [(pos[i, 0], pos[i, 1], 0, rho_0[i]) for i in range(num)]
    else:
        print('write_ply(): dim exceeds default values')
        return
    np_pos = np.array(list_pos, dtype=[('x', 'f4'), ('y', 'f4'), ('z', 'f4'), ('rho_0', 'f4')])
    el_pos = PlyElement.describe(np_pos, 'vertex')
    PlyData([el_pos]).write(str(path) + '_' + str(frame_num) + '.ply')

def write_ply2(path, frame_num, dim, num, pos, rho_0, force):
    if dim == 3:
        list_pos = [(pos[i, 0], pos[i, 1], pos[i, 2], rho_0[i], force[i]) for i in range(num)]
    elif dim == 2:
        list_pos = [(pos[i, 0], pos[i, 1], 0, rho_0[i], force[i]) for i in range(num)]
    else:
        print('write_ply(): dim exceeds default values')
        return
    np_pos = np.array(list_pos, dtype=[('x', 'f4'), ('y', 'f4'), ('z', 'f4'), ('rho_0', 'f4'), ('force', 'f4')])
    el_pos = PlyElement.describe(np_pos, 'vertex')
    PlyData([el_pos]).write(str(path) + '_' + str(frame_num) + '.ply')
...
def create_stats(stat_path):
    f = open(stat_path, 'w')
    f.write('frame'+','+'time'+','+'run_time'+','+'compression'+','+'iter_count'+','+'total_run_time'+','+'total_iter\n')
    f.close()

def write_stats(stat_path,frame,time,run_time,compression,iter_count,total_run_time,total_iter):
    f = open(stat_path, 'a')
    f.write(str(frame)+','+str(time)+','+str(run_time)+','+str(compression)+','+str(iter_count)+','+str(total_run_time)+','+str(total_iter)+'\n')
    f.close()