...
class Node:
    ...
    @ti.kernel
    def rotation(self, pos:ti.template(), dir: ti.template(), w:ti.template(), dt:ti.template(), rotate_part:ti.template()):
        theta = dt*w
        for i in rotate_part:
            if rotate_part[i] == 1:
                pos[i] = pos[i]*ti.cos(theta) + ti.math.cross(dir,pos[i])*ti.sin(theta) + dir * ti.math.dot(dir,pos[i]) * (1-ti.cos(theta))
