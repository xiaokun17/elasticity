...
class ISPH_Elastic(SPH_kernel):
    def __init__(
        ...
        search_template,
        port_lame_mu="elastic_sph.lame_mu",
        port_lame_lambda="elastic_sph.lame_lambda",
        ...
        port_dir="elastic_sph.dir",
        port_ani_mu="elastic_sph.ani_mu",
        port_act="elastic_sph.act",
        length=0,
        isAnisotropic=False,
        isMuscle=False,
    ):
        ...
        self.lame_mu = eval("self.obj." + (port_lame_mu))
        self.lame_lambda = eval("self.obj." + (port_lame_lambda))
        ...
        self.dir = eval("self.obj." + (port_dir))  # Anisotropic direction
        self.ani_mu = eval("self.obj." + (port_ani_mu))  # Anisotropic lame mu
        self.act = eval("self.obj." + (port_act))  # activation level
        self.length = length  # contraction length
        self.isAnisotropic = isAnisotropic
    ...
    def compute_P(
        ...
        for pid in range(obj.info.stack_top[None]):
            obj_output_P[pid] = (2 * self.lame_mu[pid] * obj_eps[pid]) + (
                self.lame_lambda[pid] * obj_eps[pid].trace() * I
            )
    ...
    def compute_P_Anisotropic(
        ...
        pid0 = pid2 = 0
        for pid in range(obj.info.stack_top[None]):
            C = obj_F[pid].transpose() @ obj_F[pid]
            S = ti.polar_decompose(obj_F[pid])[1]
            aTCa = (self.dir[pid].transpose() @ C @ self.dir[pid])[0]
            aTSa = (self.dir[pid].transpose() @ S @ self.dir[pid])[0]
            if aTSa == 0:
                pid0 = pid
            if aTSa == 2:
                pid2 = pid
            A = self.dir[pid].outer_product(self.dir[pid])
            obj_output_P[pid] += self.ani_mu[pid] * (1-self.signum(aTSa)/ti.sqrt(aTCa)) * obj_F[pid] @ A
        obj_output_P[pid0] = obj_output_P[pid2]
    
    @ti.kernel
    def compute_P_Muscle(
        self,
        obj: ti.template(),
        obj_F: ti.template(),
        obj_output_P: ti.template(),
        ...
        for pid in range(obj.info.stack_top[None]):
            ...
            if aTSa == 2:
                pid2 = pid
            A = self.dir[pid].outer_product(self.dir[pid])
            obj_output_P[pid] += 2*self.act[pid] * (1-self.length*self.signum(aTSa)/ti.sqrt(aTCa)) * obj_F[pid] @ A
        obj_output_P[pid0] = obj_output_P[pid2]
    ...
    def internal_loop(self, output_force):
        ...
        if self.isAnisotropic and not self.isMuscle:
            self.compute_P_Anisotropic(
                obj=self.obj,
                obj_F=self.obj_F,
                obj_output_P=self.obj_P,
            )
        if self.isAnisotropic and self.isMuscle:
            self.compute_P_Muscle(
                obj=self.obj,
                obj_F=self.obj_F,
                obj_output_P=self.obj_P,
            )
