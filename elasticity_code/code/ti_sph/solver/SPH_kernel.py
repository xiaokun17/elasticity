...
def cfl_dt(
    ...
    for i in range(obj.info.stack_top[None]):
        if bigger_than_zero(obj_size[i]):
                ...
                if background_neighb_grid.within_grid(neighb_cell_index):
                    for neighb_part in range(
                        ...
                        if dis > 1e-6:
                            ...
                            # )
                            # A_ij = obj_input_attr[pid] - nobj_input_attr[nid]
                            # obj_output_attr[pid] += coeff[None] * self.dt * nobj_volume[nid] * A_ij * W
                            obj_output_attr[pid] += coeff[None] * self.dt * nobj_input_attr[nid]

            # obj_output_attr[pid] += obj_input_attr[pid]
            obj_output_attr[pid] += (1 - coeff[None] * self.dt) * obj_input_attr[pid]
